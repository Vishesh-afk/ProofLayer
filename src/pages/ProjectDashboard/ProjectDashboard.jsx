...
const ProjectDashboard = () => {
  ...
  const canManageAssets = hasPermission(userRole, 'canImportTestimonials') || hasPermission(userRole, 'canManageUsers');
  ...
  const handleUploadAsset = async (assetData, file, onProgress) => {
    ...
    // 1. Upload to storage ONLY IF a file was provided (not an external link)
    if (file) {
      ...
      <header className="flex flex-col gap-6 px-6 py-8 md:px-10 border-b border-border bg-surface shadow-sm z-10 sticky top-0 md:static">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
             {canManageAssets && (
               <button 
                  className="flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white rounded-full px-5 py-2 text-sm font-semibold transition-all duration-200 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 border-none"
